...
const insertXmlStudentIntoDatabase = (data) => {
    return new Promise((resolve, reject) => {
        const query = 'INSERT INTO Student (student_id, Name, Gender, session_id, Class_roll, Exam_roll, Registration_no, Email, Password, Phone) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
        db.query(query, [data.student_id, data.Name, data.Gender, data.session_id, data.Class_roll, data.Exam_roll, data.Registration_no, data.Email, data.hashedPassword, data.Phone], (err, results) => {
            if (err) {
                reject(err);
            } else {
                resolve(results);
